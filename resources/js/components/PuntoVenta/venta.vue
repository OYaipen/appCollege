...
<div class="container-fluid">
            ...
            <DIV class="col-12">
                <div class="d-flex justify-content-between border-bottom mb-3 pb-2 align-items-center flex-wrap flex-md-nowra">
                     <h1>PUNTO DE VENTAS <i class="fas fa-dollar-sign"></i></h1>
                    <div class="btn-toolbar">
                       <button @click="tableToExcel('table')" 
                       class="btn btn-success mr-2" type="button" >
                        <i class="fas fa-file-excel"></i> DESC EXCEL 
                      </button>
                     <button type="submit" @click="buscar" class="btn btn-danger mr-2">SEARCH</button>
                      </div>
                </div>
            </div>
        </div>
...
<div class="input-group mb-3">
  ...
  <input type="search" class="form-control" placeholder="Buscar Producto......" v-model="busqueda">
</div>
<ul class="list-group">
  <li class="list-group-item list-group-item-light" v-for="(item, index) in filtrobuscar" :key="index">
  <a href="#" @click="agregar(item)"> {{item.descripcion}} <i class="fas fa-plus-circle"></i></a>
  </li>
</ul>
...
<div class="table-responsive-xl">
  <table ref="table" class="table table-hover table-sm  text-center">
    <thead class="thead-dark">
      <th class="icon-ventas">Producto</th>
      <th class="icon-ventas">Cantidad</th>
      <th class="icon-ventas">Fecha</th>
